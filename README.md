...
This project was refactored from a single-block script into a decoupled, professional architecture by solving five major code smells and architectural violations:
...
### 5. Violation #5: Liskov Substitution Principle (LSP) Alignment
- **The Problem:** The division logic previously controlled UI outputs directly by printing error messages to the console and returning an incorrect default value (`0`) during edge cases (division by zero), breaking the mathematical consistency expected by the contract.
- **The Refactoring:** Standardized the `Division` class to throw a `DivideByZeroException`. The main application loop now intercepts this exception via a `try-catch` block to handle UI messaging, ensuring all implementations of `IOperacion` remain entirely predictable and interchangeable.
